...
// --------------------- GLOBAL VARIABLES - START ---------------------
	...
	var cambridgeNumTickets = 0;
	var KitchenerNumTickets = 0;
	var GuelphNumTickets = 0;
	var numTickets = 0;
    ...
    // Global selectors
    // Buttons
	var reopenTicketButton = $('#reopen-ticket');
...
// --------------------- EVENT LISTENERS - START ---------------------
	...
	// label
	//$('#closed-update').on('click', fillRecentTickets); 
	...
	$('#sendText').on('click',function(){
		...
		var sendMessage = firebase.functions().httpsCallable('sendText');
		var phNum = '+1' + document.getElementById("cusPhone").value;
		var msgToSend = document.getElementById("msgToSend").value;
		console.log(phNum,msgToSend);
		sendMessage({phone: phNum, msg: msgToSend})
		  .then((result) => {
	...
	$('#closed-update').on('click', function(){ 
		//$("#search-tickets-container").load(" #search-tickets-container > *");
		$("#table-body").empty();
	...
	$('#sort-cambridge').on('click', function() {
		$("#table-body").empty();
		 cambridgeNumTickets = 0;
		 KitchenerNumTickets = 0;
		 GuelphNumTickets = 0;
		 numTickets = 0;
		var checkClosed = document.getElementById("check-closed").checked;
		database.ref('/tickets')
        .orderByChild('descOrder')
        .on("child_added", function(ticketsSnapshot) {

            var oneTicketChild = ticketsSnapshot.val();
			if (showLocation.val() == 'All' && checkClosed == true){
				createRowWithTicketReport(oneTicketChild, 'append');
			}else{
				createCambridgeRow(oneTicketChild, 'append', showLocation, checkClosed);
			}
		//$("#search-tickets-container").load(" #search-tickets-container > *");
		})
	})
...
//----------------- </REPORTS>
	...
	function createCambridgeRow(pOneTicketData, pOrder, showLocation, checkClosed) {
...
//----------------- </REPORTS>
		...
		if (showLocation.val() == 'All'){
			if((pOneTicketData.location == 'Cambridge' || 'Kitchener' || 'Guelph') && x == checkClosed)
			{
			numTickets = numTickets + 1;
			document.getElementById("search-text").textContent = numTickets + " Open Tickets in All Stores";
			...
			if((pOneTicketData.location == showLocation.val()) && x == checkClosed)
			{
				
				if(pOneTicketData.location == 'Cambridge') {
					cambridgeNumTickets = cambridgeNumTickets + 1;
					document.getElementById("search-text").textContent = cambridgeNumTickets + y + " Tickets in Cambridge";
				}
				else if(pOneTicketData.location == 'Guelph') {
							GuelphNumTickets = GuelphNumTickets + 1;
							document.getElementById("search-text").textContent = GuelphNumTickets + y +" Tickets in Guelph";
				}
				else if(pOneTicketData.location == 'Kitchener') {
							KitchenerNumTickets = KitchenerNumTickets + 1;
							document.getElementById("search-text").textContent = KitchenerNumTickets + y + " Tickets in Kitchener";
				}
                ...
                snapshot.forEach(function(snapshotChild) {
                    ...
                    for (var key in customerTickets) {
                        ...
                        .then(function(oneTicketSnapshot) {
                            ...
                            createRowWithTicketReport(oneTicketData, 'prepend'); // We use prepend to make them show from the most recent to the oldest
    ...
    function cancelDeliver() {
        deliverSectionSelector.hide();
        hideDeliverSection = true;deliverCompletedCheckboxSelector.prop('disabled', false);
